...
Posteriormente al mirar los departamentos me sorprendió que hubiera ese incremento en un determinado departamento de Justicia. En este analysis exploro un poco más algunos departamentos de Justícia y sus tendencias.
...
primera <- df %>% 
    filter(section == "4",
           department == "JUZGADOS DE PRIMERA INSTANCIA E INSTRUCCIÓN") %>% 
    mutate(text = trimws(toupper(text))) %>% 
    mutate(text = gsub("\\.$", "", text))
...
m %>% 
    ...
    labs(col = "Juzgados", x = element_blank(), y = "Anuncios",
         caption = "Fuente: Agencia Estatal del Boletín Oficial del Estado") +
    scale_x_date(labels = scales::date_format("%Y"), date_breaks = "1 year") +
    scale_y_continuous(expand = c(0, 5), limits = c(0, max(m$n) + 5)) +
    theme_bw()
```
...
ord <- df %>% 
    ...
    as_tibble()
militar <- ord %>% 
    filter(department %in% c("TRIBUNALES MILITARES TERRITORIALES", 
...
militar %>% 
    ...
    as_tibble() %>% 
    ggplot() +
    geom_col(aes(year_s, n, fill = department)) +
...
militar %>% 
    filter(department == "TRIBUNALES MILITARES TERRITORIALES") %>% 
    mutate(year_s = as.character(year(date))) %>% 
    group_by(year_s) %>% 
    count(text) %>% 
    ungroup() %>% 
...
militar %>% 
    filter(department == "JUZGADOS TOGADOS MILITARES") %>% 
    mutate(year_s = as.character(year(date))) %>% 
    group_by(year_s) %>% 
    count(text) %>% 
    ungroup() %>% 
