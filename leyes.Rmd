...
leyes %>% 
  ...
  theme_minimal() +
  labs(x = "Fecha publicación", y = "Fecha aprobación",
       col = "Tipo", 
...
leyes %>% 
  ...
  ggplot() + 
  geom_linerange(aes(y = fecha_pub, x = date, xmin = date, xmax = fecha_pub),
                 col = "grey") +
  geom_point(aes(date, fecha_pub, col = tipo)) +
  theme_minimal() +
  labs(x = "Fecha publicación", y = "Fecha aprobación",
...
La segunda publicación hay discrepancias en el sumario respecto a la publicación.
Omito las dos publicaciones a partir de ahora.

Excluyendo aquellas que se aprobaron antes de 2009:

```{r retraso-medio-filter}
leyes_retraso <- leyes_retraso[leyes_retraso$retraso >= 0, ]
leyes_retraso_medio <- leyes_retraso %>% 
  filter(fecha_pub >= as.Date("2009/01/01")) %>%
  group_by(tipo) %>%
  summarize(retraso = median(retraso, na.rm = TRUE),
            n = n()) %>%
