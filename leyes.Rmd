...
boe <- DBI::dbConnect(RSQLite::SQLite(),
...
presis <- rbind(presi, cese) %>% 
  ...
  mutate(start = if_else(is.na(start), as.Date("2009-01-01"), start),
         end = if_else(is.na(end), as.Date("2021-02-21"), end))
...
leyes_presi <- date_tipo %>% 
  mutate(presi = case_when(
    fecha_pub < presis$end[1] ~ presis$presidente[1],
    fecha_pub < presis$end[2] & fecha_pub > presis$start[2] ~ presis$presidente[2],
    fecha_pub < presis$end[3] & fecha_pub > presis$start[3] ~ presis$presidente[3],
    fecha_pub < presis$end[4] & fecha_pub > presis$start[4] ~ presis$presidente[4],
    fecha_pub < presis$end[5] & fecha_pub > presis$start[5] ~ presis$presidente[5],
    TRUE ~ ""
  ))
...
date_tipo %>% 
  ...
  droplevels() %>% 
  ggplot() +
  geom_rect(aes(xmin = start, xmax = end, ymin = 0, ymax = 35, 
