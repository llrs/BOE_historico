...
Las publicaciones suelen ser efectivas a partir del momento de publicación por eso es importante ver si hay retrasos entre aprovación y publicación. 
...
r3 %>% 
  ...
  labs(x = "Dias de retraso", y = "Publicaciones",
...
r3 %>% 
  ...
  labs(x = element_blank(), y = "Dias de retraso", 
       caption = "Fuente: Agencia Estatal del Boletín Oficial del Estado") +
  theme_bw()
...
Hay una publicación del 30 de marzo de 2010 que por fecha pone "el tres de mayo de dos mil dos"! y otras dos que no se sabe el dia que se tomaron y una con demasiadas fechas como para saber con exactitud que pasó.
...
Hay unas `r length(m)` publicaciones donde las que la diferencia es de más de un año, algunas porque hay varias fechas (error mío), otras estan bien, se aprueban en 2019 cosas de 2007 o similares.

Sorprendentemente hay publicaciones que se han aprobado el mismo día:

```{r mismo}
filter(r3, df == 0) %>% 
    select(date, section_number, department, epigraf, text) %>% 
...
r3 %>% 
  ...
  labs(x = element_blank(), y = "Dias medios de retraso", 
       caption = "Fuente: Agencia Estatal del Boletín Oficial del Estado") +
  theme_bw()
...
Aunque la tendencia es a reducir el tiempo entre aprovación y publicación, como se puede ver en el segundo gráfico sobre las fechas que tienen menos de 100 dias de retraso de mediana.
